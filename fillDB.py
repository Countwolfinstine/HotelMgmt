...
start_timestamp = time.mktime(time.strptime('2017-09-15 07:30:00', '%Y-%m-%d %H:%M:%S'))
end_timestamp = time.mktime(time.strptime('2017-09-15 22:30:00', '%Y-%m-%d %H:%M:%S'))
...
def fill_users_table(num_of_users, total_num_waiters):
    ...
    for i in range(52, num_of_users+1+52):
        ...
        with f:
            ...
            row = [email_id, uid, password, fname, authorization]
            writer.writerow(row)
    f.close()
    return
...
def temp_fill_order_table(num_of_orders, num_of_tables):
    # seperate user_id and waiter_id
    user_id = [i for i in range(4, 66)]
    waiter_id = [i for i in range(1,4)]
    waiter_id.append(user_id[5])
    del user_id[5]
    waiter_id.append(user_id[6])
    del user_id[6]
    waiter_id.append(user_id[7])
    del user_id[7]
    waiter_id.append(user_id[9])
    del user_id[9]
    waiter_id.append(user_id[12])
    del user_id[12]
    # item_id representing a particular item - unique
    item_id = [i for i in range(1, 21)]

    orders = 181
    while orders<=num_of_orders+180:
        for uid in user_id:
            for j in range(0,3):
                f1 = open(r'Orderlog.csv', 'a', newline = '')
                with f1:
                    ...
                    row = [oid, uid, iid, quantity, timestamp]
                    writer.writerow(row)
                f1.close()
                f3 = open(r'rating.csv', 'a', newline = '')
                with f3:
...
def main():
    #fill_items_table()
    #fill_users_table(50, 0)
    temp_fill_order_table(40, 5)
    #temp_fill_order_table(66,5)
main()
